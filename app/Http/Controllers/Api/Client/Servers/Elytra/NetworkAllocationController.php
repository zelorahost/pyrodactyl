...
class NetworkAllocationController extends ClientApiController
{
    ...
    public function store(NewAllocationRequest $request, Server $server): array
    {
        if (!$server->allowsAllocations()) {
            throw new DisplayException('No puedes crear asignaciones adicionales para este servidor.');
        }

        if ($server->hasAllocationLimit() && $server->allocations()->count() >= $server->allocation_limit) {
            throw new DisplayException('Has alcanzado el límite de asignaciones para este servidor.');
        }
    ...
    public function delete(DeleteAllocationRequest $request, Server $server, Allocation $allocation): JsonResponse
    {
        ...
        if ($server->allocation_limit === 0) {
            throw new DisplayException('No puedes eliminar asignaciones de este servidor: no hay un límite establecido.');
        }

        if ($allocation->id === $server->allocation_id) {
            throw new DisplayException('No puedes eliminar la asignación primaria del servidor.');
        }
